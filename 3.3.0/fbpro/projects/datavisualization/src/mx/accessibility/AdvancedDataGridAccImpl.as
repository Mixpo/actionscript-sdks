...
public class AdvancedDataGridAccImpl extends ListBaseAccImpl
{
	...
	public function AdvancedDataGridAccImpl(master:UIComponent)
	{
		...
		// set the description, shortcut properties to empty string
		// so that, fetching them doesn't throw an exception
		master.accessibilityProperties.description = "";
		master.accessibilityProperties.shortcut = "";
	...
	override public function get_accRole(childID:uint):uint
	{
		...
		if (advancedDataGrid.selectionMode == "singleCell")
		{
			...
			if (coord)
			{
				...
				if (advancedDataGrid.dataProvider is IHierarchicalCollectionView && coord.columnIndex == 0)
	...
	override public function get_accValue(childID:uint):String
	{
		...
		if (childID == 0)
		{
			...
			{
				row = advancedDataGrid.selectedIndex;
				rowStr = ", Row " + (row + 1);
				...
				if (row > -1)
				{
					...
					else
					{
						...
						if (advancedDataGrid.selectionMode == "singleCell" && advancedDataGrid.selectedCells.length > 0)
						{
							...
							if (coord)
							{
								item = getItemAt(coord.rowIndex);
								rowStr = ", Row " + (coord.rowIndex + 1);
								...
								accValue += columns[coord.columnIndex].headerText + ": " + columns[coord.columnIndex].itemToLabel(item);
							}
						}
						else
						{
			...
			else
			{
				...
				if (coord)
				{
					...
					else
					{
						...
						//if (AdvancedDataGrid.selectable == true &&
						//    AdvancedDataGrid.isItemSelected(row.data))
						{
							n = columns.length;
							for (i = 0; i < n; i++)
							{
			...
			if (advancedDataGrid.dataProvider is IHierarchicalCollectionView)
			{
				...
				if (item && view.source.canHaveChildren(item))
				{
					if (firstColumn)
					{
						...
						if (advancedDataGrid.editable.length == 0)
							accValue +=  strMOfN + str;
						else
							accValue += strMOfN ;
					}
					else
						accValue = accValue + rowStr;
				}
				else if(firstColumn)
					accValue += strMOfN;
			}
		...
		else if(advancedDataGrid.dataProvider is IHierarchicalCollectionView)
		{
	...
	override public function get_accState(childID:uint):uint
	{
		...
		if (childID > 0)
		{
			...
			if (advancedDataGrid.editable.length == 0)
			{
				...
				else if (row < advancedDataGrid.verticalScrollPosition ||
	...
	override public function get_accSelection():Array
	{
		...
		if (advancedDataGrid.selectionMode == "singleCell")
		{
			...
			for (i = 0; i < n; i++)
			{
				var coord:Object = advancedDataGrid.selectedCells[i];
	...
	override protected function eventHandler(event:Event):void
	{
		...
		switch (event.type)
		{
			...
			case AdvancedDataGridEvent.ITEM_FOCUS_IN:
			{
				if (advancedDataGrid.editable.length != 0)
				{
					var item:int = AdvancedDataGridEvent(event).rowIndex;
					var col:int = AdvancedDataGridEvent(event).columnIndex;

					Accessibility.sendEvent(advancedDataGrid,
									advancedDataGrid.columns.length * item + col + 1,
									EVENT_OBJECT_FOCUS);
