...
public class OLAPCube extends Proxy implements IOLAPCube, IEventDispatcher
{
    ...
    mx_internal function get attributeLevels():Array
    {
        ...
        for each (var dim:IOLAPDimension in dimensions)
        {
            ...
            for each (var h:OLAPHierarchy in dim.hierarchies)
            {
                ...
                {
                    a = OLAPLevel(hLevels.getItemAt(index)).attribute;
                    _attributeLevels = _attributeLevels.concat(a.allLevels.toArray());
                }
            }
        }
    ...
    mx_internal function get defaultTupleMembers():Array
    {
        if (!_defaultTupleMembers)
        {
            ...
            var aIndex:int = 0;
            for (var index:int = 0; index < attributeLevels.length ; index += 2)
            {
                var attr:OLAPAttribute = attributeLevels[index].attribute;
    ...
    public function set dimensions(value:IList):void
    {
        ...
        _dimensions = value;
        for (var dimIndex:int = 0; dimIndex < value.length; ++dimIndex)
        {
            var dim:OLAPDimension = value.getItemAt(dimIndex) as OLAPDimension;
            dim.cube = this;
    ...
    */
    ...
    public function refresh():void
    {
        ...
        if (!findDimension(measureDimensionName))
        {
            ...
            for (var mIndex:int = 0; mIndex < _measures.length; ++mIndex)
            {
                var m:OLAPMeasure = _measures.getItemAt(mIndex) as OLAPMeasure;
    ...
    private function updateDimensions(event:TimerEvent):void
    {
        if (!prepare)
        {
            ...
            prepare = true;
            dimIndex = 0;
            dispatchCubeProgress(dimIndex, dimensions.length, "Processing dimension: " + dimensions.getItemAt(dimIndex).name);
            return;
        }
    ...
    */
    private function buildCube(event:TimerEvent):void
    {
        ...
        if (progress == (total+1))
        {
            ...
            dispatchCubeProgress(progress-1, total, "Finalizing...Please wait");
            ++progress;
            return;
        }
        ...
        while (timeTaken < workDuration)
        {
            ...
            if (cubeBuilt)
                break;
            if (progress > total)
                dispatchCubeProgress(total, total, "Finalizing...Please wait");
            else
                dispatchCubeProgress(progress++, total, "Processing data Row: " + progress + " of " + total);
    ...
    private function getAxisHierarchies(axis:IOLAPQueryAxis):Array
    {
        ...
        for (var setIndex:int = 0; setIndex < sets.length; ++setIndex)
        {
            var tempSet:OLAPSet = sets[setIndex];
            var tuples:Array = tempSet.tuples;
            for (var tupleIndex:int = 0; tupleIndex < tuples.length; ++tupleIndex)
            {
                var tempTuple:OLAPTuple = tuples[tupleIndex];
                var list:IList = tempTuple.explicitMembers;
                for (var memIndex:int = 0; memIndex < list.length; ++memIndex)
                {
                    var tempHeirarchy:OLAPHierarchy = list.getItemAt(memIndex).hierarchy as OLAPHierarchy;
                    if (tempHeirarchy)
                    {
    ...
    private function validateQuery(q:IOLAPQuery):void
    {
        ...
        for (var prevAxisIndex:int = 0; prevAxisIndex < axes.length; ++prevAxisIndex)
        {
            var axisHierarchies1:Array = getAxisHierarchies(axes[prevAxisIndex]);
            for (var axisIndex:int = prevAxisIndex+1; axisIndex < axes.length; ++axisIndex)
            {
                var axisHierarchies2:Array = getAxisHierarchies(axes[axisIndex]);
                var dup:OLAPHierarchy = findDuplicateHierarchies(axisHierarchies1, axisHierarchies2);
                if (dup)
                {
                    var message:String = ResourceManager.getInstance().getString(
                        "olap", "duplicateHierarchyOnAxes", [dup.name, prevAxisIndex ,axisIndex]);
