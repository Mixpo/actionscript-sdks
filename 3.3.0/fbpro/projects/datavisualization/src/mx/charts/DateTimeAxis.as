                    ...
                    {                    	
                       ...
                       if(!firstLabelAdded)
                        ...
                        else
                        {
                            ...
                            {
                            	tmp.date = tmp.date + 7 * computedInterval;
                        		i = tmp.time;
                            }
                        	else if(_labelUnits == "hours")
                            {
                            	tmp.hours = tmp.hours + computedInterval;
                        		i = tmp.time;
                            }	
                            else if(_labelUnits == "minutes")
                            {
                            	tmp.minutes = tmp.minutes + computedInterval;
                        		i = tmp.time;
                            }
                            else if(_labelUnits == "seconds")
                            {
                            	tmp.seconds = tmp.seconds + computedInterval;
                        		i = tmp.time;
                            }
                            else if(_labelUnits == "milliseconds")
                            {
                            	tmp.milliseconds = tmp.milliseconds + computedInterval;
                        		i = tmp.time;
                            }
                            else
                        	{
                        		tmp.date = tmp.date + computedInterval;
                        		i = tmp.time;
                        	}
                        }
                    }
        ...
        switch (_minorTickUnits)
        {
            ...
            default:
            {
                     ...
                     i <= labelTop;)
                {
                    ...
                    {
                    	tmp.date = tmp.date + 7 * _minorTickInterval;
                    	i = tmp.time;
                    }
                    else if(_minorTickUnits == "hours")
                    {
                      	tmp.hours = tmp.hours + _minorTickInterval;
                   		i = tmp.time;
                    }	
                    else if(_minorTickUnits == "minutes")
                    {
                       	tmp.minutes = tmp.minutes + _minorTickInterval;
                   		i = tmp.time;
                    }
                    else if(_minorTickUnits == "seconds")
                    {
                      	tmp.seconds = tmp.seconds + _minorTickInterval;
                   		i = tmp.time;
                    }
                    else if(_minorTickUnits == "milliseconds")
                    {
                      	tmp.milliseconds = tmp.milliseconds + _minorTickInterval;
                   		i = tmp.time;
                    }
                    else
                    {
                    	tmp.date = tmp.date + _minorTickInterval;
                        i = tmp.time;
                   	} 
                }
                break;
