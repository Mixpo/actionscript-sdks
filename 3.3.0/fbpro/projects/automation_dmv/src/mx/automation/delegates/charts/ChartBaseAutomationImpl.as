...
public class ChartBaseAutomationImpl extends UIComponentAutomationImpl 
{
    ...
    {
        // in the mousedown handler of the event, the cooridnate recieved on the series is converted
        // to the base of the chart and then dispatched.
        // refer the mouseDownHanlder of chartBase.
        // but replaying of the event has the actual dragged item as the target.
        // so the replay to happen effectively, the coordinate needs to be convered back to the base of series.
        // so from the chart, make it glabal
        // from the gloabl convert this to the base of series
        // the dragged item is passed as the targetObj.
        // so the coordinate stored is to base of series.
        var chartBasedGlobalPoint:Point = chartBase.localToGlobal(inPoint);
        var seriesBasedLoaclPoint:Point = DisplayObject (targetObj).globalToLocal(chartBasedGlobalPoint);
        
        //return chartBase.contentToLocal(inPoint);
        return seriesBasedLoaclPoint;
    }
